        ...
        for (String componentName : object.components) {
            if (componentName.startsWith("builtin.")) {
                if (componentName.equals("builtin.Transform")) {
                    ...
                    gameObject.transform.position = (com.gameengine.api.math.Vector2) map.get("position");
                    gameObject.transform.localPosition = (com.gameengine.api.math.Vector2) map.get("localPosition");
                    gameObject.transform.scale = (com.gameengine.api.math.Vector2) map.get("scale");
                    ...
                    try {
                        ...
                        for (String fieldName : map.keySet()) {
                            for (Field f : cmp.getClass().getFields()) {
                ...
                try {
                    Class<? extends Component> cls = (Class<? extends Component>) Class.forName("com.game.scripts." + componentName.substring(5));
    ...
    public void fix(Array<GameObject> gameObjects) {
        for (String s : objectRefrences.keySet()) {
            try {
                ...
                Field f = cmp.getClass().getField(objectRefrenceFields.get(s));
                long ID = Long.valueOf(s.substring(4, s.lastIndexOf(".")));
                for (GameObject g : gameObjects) {
                    if (g.ID == ID) {
                        String cmpName = s.substring(s.lastIndexOf(".") + 1);
                        Component target = g.getComponent((Class<? extends Component>) Class.forName("com.gameengine.api.components." + cmpName));
                        f.set(cmp, target);
                        break;
                    }
                }
