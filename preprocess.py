...
output_file = "../api"
template_path = "../TemplateGame/core/src/main/java/com/gameengine/api"

pattern = re.compile("\s*(public|private|static)\s*(static|public|private)?\s*(\<T\>)?\s+(?!void)([a-zA-Z][a-zA-Z0-9]*)((\[\])*)\s+[a-zA-Z][a-zA-Z0-9]*\s*\([^)]*\)\s*\{\}\s*", re.MULTILINE)

# TEST1 \s*(public|static|private)?\s*(public|static|private)?\s*[a-zA-Z]?[a-zA-Z0-9_]*\s*(\[\])*\s*[a-zA-Z]?[a-zA-Z0-9_]*\s*\(?.*\)?\s*\{?.*\}?
# TEST2 (((public)\s*(void)?)?\s*([a-zA-Z][a-zA-Z0-9_]*))
    ...
    for file_path in root_path.rglob('*'):
        if file_path.is_file() and not os.path.basename(file_path) in exclude_files:
            ...
            for line in lines:
                ...
                if (
                    (l.startswith("import com.gameengine.") and not l.startswith("import com.gameengine.api.")) or
                    (l.startswith("import static com.gameengine.") and not l.startswith("import static com.gameengine.api."))
                ):
                    l = l.replace("import com.gameengine.utils.", "import com.game.")
                    l = l.replace("import com.gameengine.", "import com.game.")
                    l = l.replace("import static com.gameengine.", "import static com.game.")
