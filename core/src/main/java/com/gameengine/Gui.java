...
import com.gameengine.api.annotations.TreeEnd;
import com.gameengine.api.annotations.TreeStart;
import com.gameengine.api.audio.AudioClip;
import com.gameengine.api.components.*;
import com.gameengine.api.graphics.*;
...
public class Gui {
    ...
    private void toolbar() {
        ...
        if (Statics.editMode == 0) {
            if (Gdx.input.isButtonJustPressed(Input.Buttons.LEFT)) {
                for (GameObject object : Statics.currentProject.rootGameObject.children) {
                    Rectangle rect = rectangleFromGameObject(object);
                    if (rect.contains(Statics.mouse)) {
                        selection = object;
                        break;
                    }
                }
    ...
    private Object edit(Type type, String name, Object value) {
        ...
        if (type.equals(String.class)) {
            tempString.set(value == null ? "" : value.toString());
            ...
            if (tempVec3[0] != vec3.x || tempVec3[1] != vec3.y || tempVec3[2] != vec3.z) vec3 = new Vector3();
            vec3.x = tempVec3[0];
            vec3.y = tempVec3[1];
            vec3.z = tempVec3[2];
            return vec3;
            ...
            if (isDifferent) mat = new Matrix3(mat.val);
            return mat;
            ...
            if (isDifferent) mat = new Matrix4(mat.val);
            return mat;
            ...
            if (tempColor[0] != color.r || tempColor[1] != color.g || tempColor[2] != color.b || tempColor[3] != color.a) color = new Color();
            color.r = tempColor[0];
            color.g = tempColor[1];
            color.b = tempColor[2];
            color.a = tempColor[3];
            ...
            for (String key : engine.loadedTextures.keySet()) {
                ...
                if (tex == newTextureAsset) name0 = key;
            }
            ...
            for (String key : engine.loadedAudioClips.keySet()) {
                ...
                if (audioClip == newAudioClip) name0 = key;
            }
                ...
                if (nodeOpen) {
                    ...
                    for (Field field : component.fields) {
                        if (field.isAnnotationPresent(TreeStart.class)) {
                            TreeStart treeStart = field.getAnnotation(TreeStart.class);
                        ...
                        if (treeStack.isEmpty() || treeStack.peek()) {
                            try {
                                ...
                                if (List.class.isAssignableFrom(typeC)) {
                                    ...
                                    for (int j = 0; j < list.size(); j++) {
                                        ...
                                        ImGui.text(Integer.toString(j));
                                        ImGui.sameLine();
                                        if (ImGui.button("Remove##ListRemoveButton" + field.getName() + j)) remove = j;
                                        list.set(j, edit(componentType, field.getName() + j, componentTypeC.cast(o)));
                                    }
                                    if (remove != -1) list.remove(remove);
                                    ImGui.spacing();
                                ...
                                if (type.equals(Button.class)) {
                                    ...
                                    if (ImGui.collapsingHeader("Tint")) {
                                        // TODO editField("tintColorsR", particleEffect);
                                        // TODO editField("tintColorsG", particleEffect);
                                        // TODO editField("tintColorsB", particleEffect);
                            ...
                            if (field.isAnnotationPresent(TreeEnd.class) && !field.getAnnotation(TreeEnd.class).isHeader()) ImGui.treePop();
                        }
            ...
            if (ImGui.beginPopup("AddComponentPopup")) {
                ...
                for (Class<? extends Component> component : Statics.currentProject.components) {
                    if (component != Transform.class) {
                        if (ImGui.selectable(component.getSimpleName(), false, 0, 150, 18)) {
                            if (component.getModule() == Transform.class.getModule()) {
                                try {
                                    selectedGameObject.addComponent(component.getDeclaredConstructor().newInstance());
                        ...
                        if (component == Camera.class || component == AudioSource.class) ImGui.separator();
                    }
                }
    ...
    public void render() throws IOException, ClassNotFoundException {
        ...
        if (toggleBuildWindow.get()) {
            ...
            ImGui.begin("Build", toggleBuildWindow, ImGuiWindowFlags.NoResize | ImGuiWindowFlags.NoDocking);
            ...
            ImGui.setCursorPos(ImGui.getCursorPosX(), ImGui.getWindowSizeY() - 40);
        ...
        if (ImGui.beginPopup("CreateScriptPopup")) {
            ...
            if (ImGui.button("Create")) {
                ImGui.closeCurrentPopup();
                currentProject.path.child("Assets").child(fileName.get() + ".java").writeString("import com.gameengine.api.Component;\n\npublic class " + fileName.get() + " extends Component {\n\n    public void start() {\n\n    }\n    public void update() {\n\n    }\n\n}", false);
                refresh();
            }
            isAnyWindowHovered |= ImGui.isWindowHovered();
            ImGui.endPopup();
